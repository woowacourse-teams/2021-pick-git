...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("select p from Post p left join fetch p.user where p.id < :postId order by p.createdAt desc")
    List<Post> testHomeFeed(@Param("postId") Long postId, Pageable pageable);
