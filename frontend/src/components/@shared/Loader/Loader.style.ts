...
export const LoadingDots = styled.div<{ cssProp?: CSSProp }>(
  ({ cssProp }) => `
    ${cssProp}
  `
);
...
export const Spinner = styled.div<{ size: string; loaderColor?: string; cssProp?: CSSProp }>(
  ({ theme, size, loaderColor, cssProp }) => css`
    width: ${size};
    height: ${size};
